...
def show_result(num_epoch, show = False, save = False, path = 'result.png', isFix=False):
    z_ = torch.randn((5*5, 100)).view(-1, 100, 1, 1)  # 生成一个25*100*1*1的tensor，认为输入为25个一组，100张1*1的图片
    z_ = Variable(z_.cuda(), volatile=True)

    G.eval()
    if isFix:
        test_images = G(fixed_z_)  # 输出为25个一组的1维的64*64的图片
    else:
        test_images = G(z_)
    G.train()

    size_figure_grid = 5   # 画5*5的图
    fig, ax = plt.subplots(size_figure_grid, size_figure_grid, figsize=(5, 5))
    for i, j in itertools.product(range(size_figure_grid), range(size_figure_grid)):  # 让坐标轴不可见
    ...
    for k in range(5*5):
        ...
        j = k % 5
        ax[i, j].cla()
        ax[i, j].imshow(test_images[k, 0].cpu().data.numpy(), cmap='gray')  # 取出第k张图片的第1维(实际只有一维)
...
def show_train_hist(hist, show = False, save = False, path = 'Train_hist.png'):
    ...
    plt.legend(loc=4)  # 显示图例，在4号位置(lower right)
    plt.grid(True)
    plt.tight_layout()
